...
{{- $certlog := lookup "v1" "Secret" .Release.Namespace ( printf "%s-certs" (include "elasticsearch.uname" . ) ) -}}
{{- if $certs -}}
tls.crt: {{ index $certs.data "tls.crt" }}
tls.key: {{ index $certs.data "tls.key" }}
ca.crt: {{ index $certs.data "ca.crt" }}
log.crt: {{ index $certlog.data "log.crt" }}
log.key: {{ index $certlog.data "log.key" }}
{{- else -}}
...
{{- $certlog := genSignedCert "logstash-logstash" nil (list "logstash-logstash.local.svc") 365 $ca -}}
tls.crt: {{ $cert.Cert | toString | b64enc }}
tls.key: {{ $cert.Key | toString | b64enc }}
ca.crt: {{ $ca.Cert | toString | b64enc }}
log.crt: {{ $certlog.Cert | toString | b64enc }}
log.key: {{ $certlog.Key | toString | b64enc }}
{{- end -}}
{{- end -}}
